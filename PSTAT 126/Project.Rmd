---
title: \vspace{3in} "Valuing Real Estates and Concrete Strengths" 
...
This report is split into two components. The first component serves to evaluate the relationship between real estate valuation metrics and real estate prices. In particular, this report investigates how real estate valuations collected from Sindian District, New Taipei City, Taiwan during the period of June 2012 to May 2013 are affected by these 6 variables: the transaction date, the house age, the distance to the nearest Metro station, the number of convenience stores within walking distance, and the latitude & longitude coordinates. The second component of this report investigates how the concrete compressive strength is affected by these 8 concrete components: cement, blast furnace slag, fly ash, water, superplasticizer, coarse aggregate, fine aggregate, and age. 
...
Before we dove deep into asseting how would each of the predictor variables: transaction date, house age, number of convenience stores within walking distance, and latitude affect our response variable: the house price, we expected to see varying relationship among the response and each of the predictor variables. For instance, we predicted the house price of the unit area to have a positive correlation with the transaction date since global real estate prices have been on the rise ever since the end of the Great Recession in June 2009.  We also expected age to have a positive association with price because the Sindian district is an urban district so newer developments tend to be priced with a premium. Additionally, we expected the number of convenience stores within walking distance to have a positive correlation with the house price of the unit area since people typically are more willing to pay more in exchange for convenience. As for the latitude and house price of the unit area, we expected an insignificant relationship due to the district being surrounded by a river at both latitudinal ends and because of waterfront property typically having much higher valuations.  
...
From our exploratory analysis using the Added-Variable plots, we see that when accounting for the other predictors: Age, Stores, and Latitude, Transaction date does not appear to be important when included in our model. Age, Stores, and Latitude all appear to be useful.  
...
Since the p-value for the Global F-Test is relatively small, we could conclude that at least one of our predictors has a relationship with the price when taking into account the other predictors.  As for marginal p-values, we see that Age, Stores, and Latitude are significant with respect to their relationship with the price when considering the other predictors.

* If TDate, Stores, and Latitude are held constant, a one year increase in age results in an estimated average decrease of $-3.020e^{-1}$ house price of unit area.  
* If TDate, Age and Latitude are held constant, a one store increase in results in an estimated average increase of 1.929 house price of unit area. 
* If TDate, Stores, and Age are held constant, a one-degree increase in latitude results in an estimated average increase of $4.078e^2$ house price of unit area.
...
```
The t-value for Longitude is -0.256. Since the p-value (0.7983) for Longitude is relatively large when Metro is included, we fail to reject the null hypothesis that longitude does not have a relationship with Price when also accounting for Metro in the model.  From this, we can conclude that including both Metro and Longitude is not useful in our model.  However, since both predictors are significant on their own, we will use the fact that the model with Metro included has a higher r-squared value (0.582) than the model with Longitude (0.542) to choose it as the better model.  This is also confirmed by the AIC for Metro being the lower than Longitude when starting with the original four predictors followed by Longitude not having a lower AIC than none when Metro is included in the model.
...
Between the models (1) and (2), we prefer model (2) due since the addition of Metro yields a higher R-Squared value, 0.545. This means that 54.5% of the variability is explained by the model compared to the 50.15% variability that is explained by the model using Stores. We used R-Squared as our deciding metric because (1) and (2) are not submodels of each other and contain the same number of predictors.  
...
As you can see, transforming Metro leads to a significantly better r-squared value, improving the linearity of our model. Next, we transform the response. However, to determine which transformation should be applied, we used the Box-Cox transformation. 
...
plot(fitmetro)
```
The variable age, the house age in years, and the variable TDate, the transaction date, are measures of time so a transformation will not be very useful for understanding our model.  We tried log transform metro since it is a measure of distance and cannot be less than or equal to 0. Metro also has a wide range, so a log transformation is likely to be appropriate. After doing a likelihood ratio test on the metro, we see that the p-value for doing a log transformation is 0.63, which is relatively large compared to any standard significance level, allowing us to confidently transform metro. 
...
```
The results gave us a combination of $Y = 29.03 + 0.105X1 + 0.239X5 + 0.113X8 + 0.086X2 - 0.218X4 + 0.069X3$. However, before we could assume these predictors will produce the best combination, we must perform diagnostic checks for influential cases and linear regression assumptions (linearity, independence, normality, and constant variance). 
